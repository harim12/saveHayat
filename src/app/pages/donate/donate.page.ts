...
export class DonatePage implements OnInit {
  ...
  this.donation = this.formBuilder.group({
    bloodGroup: ['', [this.bloodGroupValidator]], 
    firstName:['',[Validators.required]],
    lastName:['',[Validators.required]],
    date:[''],
    hospitalAdress:['',[Validators.required]],
  ...
  async donate(){
    ...
    if(this.donation.valid){
      this.donationService.addDonation(this.donation.value)
      .then((data)=>{
        this.donation.reset();
  ...
  //pour entrer le format de blood group valid comme A+ ou A-
   bloodGroupValidator(control: FormControl) {
    ...
    return valid ? null : { invalidBloodGroup: true }; // return null if valid, else return an error object
  }
